...
// Launches a client-session task on the IO scheduler, ensuring it is started
// before control returns to the caller so that the IO loop is ready to
// service the socket.
static void SpawnClientTask(exec::async_scope& scope,
                            EpollContext::scheduler io_sched,
                            CmdContext::scheduler cmd_sched, int server_fd,
                            Server& server, CommandRegistry& registry) {
  auto task = handle_client(io_sched, cmd_sched, server_fd, server, registry);
  scope.spawn(stdexec::starts_on(io_sched, std::move(task)));
}
...
TEST(ClientSessionTest, PingRoundTrip) {
  ...
  io_thread.join();
  cmd_thread.join();
  scope.request_stop();  // help scope drain
  ::close(server_fd);
}
